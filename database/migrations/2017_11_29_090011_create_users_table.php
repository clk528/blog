...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->integer('role', false,false)->comment('角色');
            $table->string('email', 50)->unique()->comment('邮箱');
            $table->string('github', 50)->unique()->comment('github');
            $table->string('weibo', 50)->unique()->comment('微博');
            $table->string('facebook', 50)->unique()->comment('脸书');
            $table->integer('status', false,false)->comment('状态:1、正常，2、冻结');
            ...
            $table->integer('login_time',false,false)->comment('登录时间');
        });
    }
